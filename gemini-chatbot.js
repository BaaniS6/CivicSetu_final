class GeminiChatbot {
    ...
    showApiKeyModal() {
        ...
        // Add event listeners
        // Note: button[0] is "Copy to Input", button[1] is "Save & Continue", button[2] is "Cancel"
    ...
    async sendMessage(userMessage, chatBoxId) {
        ...
        if (this.isLoading) {
            return; // Prevent duplicate requests while one is in flight
        }
        ...
        try {
            ...
            if (error.message.includes('429')) {
                this.addBotMessage("⏳ Rate limit reached. The free Gemini API allows ~15 requests per minute. Please wait a minute and try again.", chatBoxId);
